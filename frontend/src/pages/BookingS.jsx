...
export default function Bookings() {
  ...
  const [form, setForm] = useState({
    stationId: "", date: "", startTime: "", duration: "1"
  })
  ...
  // ✅ Stations aur bookings fetch karo
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (station && form.duration) {
      setAmount(station.pricePerHour * Number(form.duration))
    }
  ...
  const handleBooking = async () => {
    ...
    if (!form.stationId || !form.date || !form.startTime || !form.duration) {
          ...
          {showForm && (
            <div style={{
                  ...
                  >
                    ...
                    {stations.filter(s => s.isActive).map(s => (
                      <option key={s._id} value={s._id}>
                        {s.name} — {s.location} (₹{s.pricePerHour}/hr)
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <button onClick={handleBooking} style={{
                marginTop: "20px", padding: "11px 28px", borderRadius: "8px",
                border: "none", background: "linear-gradient(135deg, #0284c7, #0ea5e9)",
                color: "#fff", fontSize: "13px", fontWeight: "700", cursor: "pointer",
                boxShadow: "0 4px 12px rgba(2,132,199,0.3)",
              }}>
                Confirm Booking ✓
                ...
                <tbody>
                  {bookings.map((b, i) => (
                    <tr key={b._id} style={{ borderBottom: i < bookings.length - 1 ? "1px solid #f8fafc" : "none" }}>
                      <td style={{ padding: "13px 12px", fontSize: "13px", fontWeight: "600", color: "#0284c7" }}>
                        {b.stationId?.name || "N/A"}
                      </td>
                      <td style={{ padding: "13px 12px", fontSize: "13px", color: "#374151" }}>
                        {b.stationId?.location || "N/A"}
                      </td>
                      <td style={{ padding: "13px 12px", fontSize: "13px", color: "#64748b" }}>
                        {new Date(b.startTime).toLocaleString()}
                      </td>
                      <td style={{ padding: "13px 12px", fontSize: "13px", color: "#64748b" }}>
                        {new Date(b.endTime).toLocaleString()}
                      </td>
                      <td style={{ padding: "13px 12px", fontSize: "13px", fontWeight: "700", color: "#0f172a" }}>
                        ₹{b.amount}
                      </td>
