  ...
  useEffect(() => {
    socket.on("newBooking", (data) => {
      setNotifications(prev => [{
        id: Date.now(),
        message: data.message || "New booking created!",
  ...
  return (
    <Layout title="Overview">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px", flexWrap: "wrap", gap: "8px" }}>
        <select style={{ padding: "8px 16px", borderRadius: "8px", border: "1px solid #e2e8f0", background: "#fff", fontSize: "13px", color: "#374151", fontWeight: "600", cursor: "pointer", outline: "none" }}>
      ...
      {/* ✅ Responsive top row */}
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : isTablet ? "1fr" : "1fr 2fr", gap: "20px", marginBottom: "20px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #e8ecf0", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            ...
            <button style={{ padding: "6px 14px", borderRadius: "7px", border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: "12px", color: "#374151", cursor: "pointer", fontWeight: "600" }}>View chargers</button>
          </div>
              ...
              <svg viewBox="0 0 36 36" style={{ transform: "rotate(-90deg)", width: "100px", height: "100px" }}>
                {(() => {
                  ...
                  return chargerStatus.map((s, i) => {
                    ...
                    const el = (<circle key={i} cx="18" cy="18" r="14" fill="none" stroke={s.color} strokeWidth="4" strokeDasharray={`${pct * 0.879} 87.9`} strokeDashoffset={-offset * 0.879} />)
                    offset += pct
                    return el
                  })
                })()}
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: "16px" }}>
          ...
          <StatsCard title="Sessions" value={`${48 + liveBookings}`} icon="🔌" trend="5%" trendUp={false} />
        </div>
      </div>

      {/* Live Notifications */}
      {notifications.length > 0 && (
        <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #e8ecf0", padding: "20px", marginBottom: "20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
            <h3 style={{ margin: 0, fontSize: "15px", fontWeight: "700", color: "#0f172a" }}>🔔 Live Notifications</h3>
            <button onClick={() => setNotifications([])} style={{ fontSize: "12px", color: "#94a3b8", background: "none", border: "none", cursor: "pointer" }}>Clear all</button>
          </div>
          {notifications.map((n) => (
            <div key={n.id} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #f1f5f9" }}>
      ...
      {/* Bar Chart */}
      <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #e8ecf0", padding: "24px", marginBottom: "20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "8px" }}>
          <div style={{ display: "flex", gap: "8px" }}>
            {["Fees collected", "Energy used", "Sessions"].map((tab, i) => (
              <button key={tab} style={{ padding: "6px 14px", borderRadius: "7px", border: "none", cursor: "pointer", fontSize: "12px", fontWeight: "600", background: i === 0 ? "#0f172a" : "#f8fafc", color: i === 0 ? "#fff" : "#64748b" }}>{tab}</button>
            ))}
          </div>
          {!isMobile && (
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <span style={{ fontSize: "12px", color: "#64748b", fontWeight: "500" }}>Apr 2025 – Mar 2026</span>
              <button style={{ padding: "6px 14px", borderRadius: "7px", border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: "12px", fontWeight: "600", color: "#374151", cursor: "pointer" }}>Export CSV</button>
            </div>
          )}
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: "8px", height: "160px" }}>
          {barData.map((val, i) => (
            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", height: "100%" }}>
              <div style={{ width: "100%", borderRadius: "6px 6px 0 0", background: "linear-gradient(180deg, #38bdf8, #0284c7)", height: `${(val / maxBar) * 100}%`, transition: "opacity 0.2s", cursor: "pointer" }}
                onMouseEnter={e => e.currentTarget.style.opacity = "0.8"}
                onMouseLeave={e => e.currentTarget.style.opacity = "1"}
                title={`$${val}`}
              />
              {!isMobile && <span style={{ fontSize: "10px", color: "#94a3b8" }}>{months[i]}</span>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #e8ecf0", padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "8px" }}>
          ...
          {!isMobile && (
            <div style={{ display: "flex", gap: "8px" }}>
              <select style={{ padding: "6px 12px", borderRadius: "7px", border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: "12px", color: "#374151", cursor: "pointer", fontWeight: "600" }}><option>All Chargers</option></select>
              <select style={{ padding: "6px 12px", borderRadius: "7px", border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: "12px", color: "#374151", cursor: "pointer", fontWeight: "600" }}><option>Last week</option><option>Last month</option></select>
              <button style={{ padding: "6px 14px", borderRadius: "7px", border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: "12px", fontWeight: "600", color: "#374151", cursor: "pointer" }}>Export CSV</button>
            </div>
          )}
        </div>
        {/* ✅ Mobile mein scroll hoga table */}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "600px" }}>
            ...
            <tbody>
              {recentSessions.map((s, i) => (
                <tr key={s.id} style={{ borderBottom: i < recentSessions.length - 1 ? "1px solid #f8fafc" : "none" }}>
